  ...
  useEffect(() => {
    if (tutors.length > 0) {
      const fetchSessions = async () => {
        ...
        setApprovedTutors(
          tutors.filter((tutor) => approvedTutorEmails.has(tutor.email))
        );
      };
      ...
      {approvedTutors.length > 0 ? (
        approvedTutors.map((tutor) => (
          <Link key={tutor.email} to={`/TutorDetails/${tutor.email}`}>
            <div className="bg-gray-100 shadow p-4 w-40 rounded-xl border border-[#a0f0d5] mx-3 flex flex-col items-center">
              <img
                src={tutor.userImg}
                className="w-20 h-20 rounded-full object-cover"
                alt={tutor.name || "Tutor"}
              />
              <h6 className="text-center text-base mt-2 font-medium">
                {tutor.name || "Unknown"}
              </h6>
        ...
        <p className="text-center text-gray-500">
          Data fetching loading Or No approved tutors found.
        </p>
      )}
    </Marquee>
  </div>
